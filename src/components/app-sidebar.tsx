                  ...
                  >
                    ...
                    <div className="grid flex-1 text-left text-sm leading-tight">
                      ...
                      <div>
                        <ButtonGroup>
                          {user.isLoading ? (
                            <Badge className="w-12" variant="loading" />
                          ) : (
                            <Badge className="text-purple-500">{user.data?.user.role}</Badge>
                          )}

                          <Badge className="text-blue-500 font-mono font-bold">
                            {appVersion.isLoading ? "..." : appVersion.data?.appVersion}
                          </Badge>
                        </ButtonGroup>
                      </div>
                    </div>
...
export const UserButtonCustom = ({ type = "sidebar" }: { type?: "navbar" | "sidebar" }) => {
      ...
      classNames={
        type === "sidebar"
          ? {
              ...
              trigger: {
                base: "bg-transparent text-white hover:bg-sidebar-accent hover:text-sidebar-accent-foreground h-12",
                user: {
                  avatar: {
                    fallback: "bg-primary text-primary-foreground",
                    base: "size-9",
